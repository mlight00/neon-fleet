...
//  스테이지 기록은 stageId + stageVersion 으로 묶는다: stages[id].versions[version] = { cleared, attempts, bestSurvivors, bestTime }.
//  코스 배치를 고치면(stages.js 의 version 상향) 새 버전 칸에 따로 쌓이므로 옛 기록과 섞이지 않는다.
...
//  버전 키: 1 이상의 정수만. 그 밖(문자열·0·소수·NaN)은 1 로 본다.
//  단, 이렇게 1 로 본 잡키가 실재하는 버전 1 기록을 덮으면 안 된다(rawVersions 참조)
const verKey = (v) => { const n = Math.trunc(Number(v)); return Number.isFinite(n) && n >= 1 ? String(n) : '1'; };
//  원문 키가 이미 정규 버전 키('1','2',…)인가 — 정규 키를 먼저 채우기 위한 판정
...
function rawVersions(s) {
  ...
  if (isPlainObject(s.versions)) {
    const out = {};
    const junk = [];
    //  1차: 정규 버전 키만 자기 자리에 채운다. 비객체·빈 객체는 '빈 칸'으로 본다(찬 칸으로 오인해 옛 기록 귀속을 막지 않게)
...
export function createSave3(storage) {
  ...
  return {
    get: () => data,
    //  한 코스 버전의 기록(기본 버전 1). 없는 스테이지·버전은 기본값
    getStage: (id, version) => ({ ...STAGE_DEFAULTS, ...(data.stages[String(id)]?.versions?.[verKey(version ?? 1)] ?? {}) }),
    //  그 스테이지의 모든 버전 기록 사본 { '1': {...}, '2': {...} } — 옛 기록 보관 확인용
    getStageVersions: (id) => {
      const v = data.stages[String(id)]?.versions ?? {};
      return Object.fromEntries(Object.entries(v).map(([k, r]) => [k, { ...STAGE_DEFAULTS, ...r }]));
    },
    //  깊은 병합: 해당 스테이지의 해당 버전만 갱신, 다른 버전·다른 스테이지 기록은 그대로
    updateStage: (id, patch, version) => {
      const k = String(id), vk = verKey(version ?? 1);
