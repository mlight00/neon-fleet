...
export const BAL3 = deepFreeze({
  ...
  squad: {
    ...
    //  r4.8 보스전 밀집 대형(difficulty 표의 bossHw 가 있는 줄 = 게임 화면 brutal): 보스가 나온 STEP 부터 대형 반폭 상한을
    //   지금 반폭에서 bossHw 까지 이 빠르기(px/s)로 줄인다(한 번에 뭉개지지 않게 — 100명 130 → 64 약 0.7초, 보스 하강 2.3초 안에 끝난다).
    //   승리 뒤(8번 보너스전)에는 같은 빠르기로 되돌린다
    bossHwRate: 90,
  ...
  midBossSec: 12,
  //  r4.10 중간 보스(2·5·8·11·14·17·20·23 — 게임 화면 줄): 그 판 일반 적 그림(courses.STAGE_END 의 look)을 크게 키운 강한 적 1체. **보스와 다르다** — 탄막·광분·고유 공격·
  //   페이즈·소환·접촉 사격 없음. 행동은 한 가지: 정지 거리에서 좌우로 움직이다 가끔 붉은 경보 줄(돌진할 길)을 띄운 뒤 그 줄로 돌진했다 제자리로.
  //   label    머리 위 이름표 글(렌더 — 체력 막대 위)
  //   scale    크기 = 그 적 종류 표 반지름(BAL3.enemies[kind].r) × 배율 — 판정 반지름·그림(일반 적 그림 규칙 r × 2.4 × 그림 배율)이 함께 커진다(일반 적의 2.2~2.6배)
  //   holdAhead 정지 거리(px) — 도로 보스(420)보다 가까이: 머리 위 이름표·체력 막대가 HUD 띠 아래(화면 y ≈ 190)에 오게
  //   crush    돌진에 치인 피해 풀(× 줄의 touchDmg 배수 = 게임 줄 18): 경보 줄 안 병사에게 앞줄부터 체력만큼 나눠 뺀다 — 현상금 적과 같은 규모(병사 체력 2 약 9명)
  //   charge   first(자리 잡은 뒤 첫 경보까지) · every(돌아온 뒤 다음 경보까지) · tele(경보 — 광역 경보 규칙 그대로 붉은 줄) · speed(돌진 px/s) · back(제자리로 px/s) ·
  //            retry(피할 곳이 보장되는 줄을 못 찾으면 다시 볼 때까지). 피할 수 있음 보장 = 보스 광역과 같은 안전 상자(bossatk.planCharge — 부대 폭 + 48 · 닿는 거리 = 250 × 경보 × 0.8)
  midBoss: { label: '중간 보스', scale: { grunt: 2.6, rusher: 2.4, shooter: 2.2 }, holdAhead: 360, crush: 6,
             charge: { first: 1.5, every: 2.4, tele: 0.8, speed: 900, back: 420, retry: 0.25 } },
