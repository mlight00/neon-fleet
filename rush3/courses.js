...
//  현상금 적 배치 표(r4.7) — 판 번호 → [[발동 z, x], ...]. 규칙은 makeCourses 끝의 주석
const BOUNTY_AT = Object.freeze({
  4: [[6350, 240]], 5: [[6750, 240]], 6: [[4650, 240]], 7: [[2650, 240]], 8: [[2450, 240]], 9: [[7100, 240]], 10: [[7150, 240]], 11: [[8150, 240]], 12: [[8750, 240]],
  13: [[2250, 170], [6950, 310]], 14: [[4650, 170], [7550, 310]], 15: [[4450, 170], [6050, 310]], 16: [[4850, 170], [7750, 310]],
  17: [[4350, 170], [7350, 310]], 18: [[4850, 170], [7950, 310]], 19: [[2250, 170], [7550, 310]], 20: [[4850, 170], [7500, 310]],
...
export function makeCourses({ coverZFor }) {
  ...
  //  현상금 적(r4.7, 이사님 지시 2026-09-26 — 게임 화면 줄 brutal 에서만 stages.buildStage 가 스폰 1체로 붙인다. 배수 1 줄은 읽지 않는다, version 불변).
  //   [발동 z, x] — 판마다 1체, 13번부터 2체. 배치 규칙(계약서 r4.7 (c)): 적이 나온 뒤 부대에 닿기까지 사선이 지나는 트랙 구간
  //   [z, firepower.bountyWindow(z).b] 안에 게이트·보급 통(발판 포함)·벽·차폐물이 없고, 끝이 보스 z − 150 앞. 그런 z(50px 격자, 1,500 부터) 중
  //   같은 때 화면에 나오는 다른 적 수가 가장 적은 곳(같으면 뒤쪽), 2체는 둘째가 첫째 창이 끝난 뒤. x 는 1체 240 · 2체 170/310(나온 뒤 부대 x 를 따라간다).
  //   이 규칙으로 다시 뽑아 표와 비교하는 도구 = tools/place-bounties.mjs(봇을 돌리지 않는다 — 판 정의만 본다)
  for (const [id, list] of Object.entries(BOUNTY_AT)) C[id].bounties = list.map(([z, x]) => ({ z, x }));
