...
#
#   python tools/nocache-server.py [포트] [폴더] [호스트]   (기본 9098 · 현재 폴더 · 127.0.0.1)
#
#  스마트폰 실기(같은 Wi-Fi)용은 호스트를 0.0.0.0 으로 — 그때만 LAN 에 열린다(처음 띄울 때 Windows 방화벽 허용 창이 뜬다).
#
...
if __name__ == '__main__':
    ...
    print('serving on http://%s:%d (no-cache) root=%s' % (host, port, root or '.'), flush=True)
