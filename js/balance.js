...
export const BAL = {
  ...
  // 함선 진화 (부록 §1): 드론 수 임계값으로 승급, 60% 밑으로 떨어지면 강등
  evolution: {
    // 진화에 더 많은 드론 필요 (스테이지 5쯤 최종 티어 도달 목표)
    thresholds: [0, 100, 300, 750, 1600, 2800],  // T1 스카웃 ~ T6 타이탄
    names: ['스카웃', '인터셉터', '스트라이커', '커리어', '드레드노트', '타이탄'],
    dpsMult: [1.0, 1.2, 1.45, 1.75, 2.05, 2.4],
    demoteRatio: 0.6,
  },
  ...
  boss: {
    hp: 11700,         // 최소 HP (1.3배 난이도 상향)
    hpPerDrone: 20,    // 보스 등장 시 max(hp, 최대편대기록 x 이 값) — 어떤 편대든 보스전 4~6초 유지
    radius: 60,
