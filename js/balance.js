...
export const BAL = {
  ...
  coin: {
    ...
    dropChance: 0.05,      // 일반 몹 코인 드랍 확률(이사: 모든 적이 아니라 5%만). 정예★·중간보스·섹터보스는 항상.
  ...
  charge: {
    ...
    //  §G-40(이사님 결정 2026-08-13): 1.05 → **0.70** (현재의 2/3).
    //   ⚠️이 값은 그림 폭만이 아니라 **판정 폭**이기도 하다 — 적·보스 명중, 3단계 적탄 소거가 전부 이걸 쓴다.
    //    즉 이건 **난이도 변경**이다. "난이도 달라지는 건 상관 없다"는 이사님 승인 하에 바꿨다.
    //   ⚠️3D 빔은 이 값에서 유도되므로(§G-40 lanceWidthScreen) 따로 손댈 필요가 없다.
    //  §G-40(이사님 결정 2026-08-13): 0.70 → **1.40**.
    //   3D 만 2배로 넓히던 것을 없애는 대신(LANCE_3D_WIDTH_MULT 2.0 → 1.0) **기본 폭을 그만큼 키운다.**
    //   결과: 2D 그림 = 3D 그림 = 판정이 **한 숫자**. 어느 시점에서 보든 보이는 대로 맞는다.
    widthShipMult: 1.40,               // 기함 폭 대비 빔 배수(1.0=기함 폭과 동일)
    //  §G-40(이사님 결정 2026-08-13): **판정 = 그림.**
    //   그림 반폭 = halfW × (0.7 + 0.5·p) 이고, 판정은 발사 순간(p=1)에 한 번 계산된다 → 배수 **1.2**.
    //  ⚠️3D 도 같은 폭으로 그린다(LANCE_3D_WIDTH_MULT = 1.0) → 2D·3D·판정이 전부 일치한다.
    //  ⚠️`LANCE_3D_WIDTH_MULT` 를 1 에서 벗어나게 바꾸면 이 일치가 깨진다.
    //   게임 판정은 렌더 상수를 import 하지 않는다(경계 규칙) — 대신
    //   `tests/g40-lance-width.test.mjs` 의 `G40-HIT-MATCHES-3D-DRAW` 가 어긋남을 잡는다.
    hitWidthMult: 1.2,
  ...
  boss: {
    ...
    // 섹터별 추가 HP 배수(없으면 1). 기본 곡선(1+(섹터-1)×0.22)은 섹터 1이 1.0이라 손댈 자리가 없어서,
    // 특정 섹터만 따로 조정하려고 둔 표. 섹터 1 보스가 너무 쉽게 죽어 2배(이사).
    sectorHpMult: { 1: 2 },
    // 보스 HP에 격납고 영구 강화(공격력·연사력)를 얼마나 반영할지. 0=옛 동작(무시), 1=완전 반영.
    // 이게 없으면 강화가 쌓일수록 보스만 일방적으로 물러진다(섹터 1 보스가 10초에 죽던 진짜 원인).
    hangarWeight: 1,
    radius: 60,
  ...
  sector: {
    ...
    extraSeconds: 15,  // 모든 노드에 추가할 플레이 시간(초). 보스 노드는 보스 출현 전 트랙이 그만큼 길어짐. 콘텐츠는 늘어난 트랙에 고르게 펼침(빈 구간 없음)
  ...
  gate1: {
    ...
    loadout: {
      ...
      //  ⚠️**이 값은 게임 좌표다. 시각 조정용으로 절대 바꾸지 마라.**
      //   `entities._spawnWeaponShots` 가 `this.x + hx + m.x` 로 탄을 생성하고,
      //   `main.spawnResonance` 가 레일 시작 x 에 그대로 쓴다 — 바꾸면 탄 궤적·충돌 지점이 달라진다.
      //   §G-23 에서 "보조 무기가 선체 밖에서 발사되는 것처럼 보인다"는 시각 지적을 고치려고
      //   22 → 8 로 낮췄던 적이 있다(§G-28 에서 복원). 그 시각 문제는 **렌더 전용 경로**에서 푼다:
      //   포구 섬광을 선체 안으로 당기고, 탄의 첫 프레임만 포구에서 실제 위치로 보간한다(§G-28 P0-1).
