...
export const INSTANT_ART = [
  'A1',                                                   // 기함 T0 EMBER 선체 — 출격 순간부터 화면에 있다(§G-48)
  ...
  //  ⚠️A2·A3 는 여기서 뺐다 — preload 는 6개 안팎을 넘으면 서로 슬롯을 다퉈 효과가 사라진다.
  //   호위 순양함과 승급 선체는 몇 초 뒤에 나오지만 발사체는 **첫 프레임부터** 나온다. 우선순위가 다르다.
  //   둘 다 1파(FIRST_CONTACT)에 그대로 있어 곧바로 이어서 받는다.
  //  ⚠️예전 주석은 A1~A3 를 "적"이라고 적어 두었는데 사실이 아니다 — A 계열은 **아군 기함 6등급**이다.
];

/** 1파 — 출격 직후 즉시 보이는 것: 기함 선체·무기 마운트·아군 발사체·초기 적·픽업. */
const FIRST_CONTACT = [
  //  §G-48: 선두는 **출격 순간 실제로 화면에 있는 것**이어야 한다.
  //   예전엔 H2_BASE_FRAME(옛 T2 전용 선체)이 1번이었는데, 플레이어는 T0 로 출격하므로
  //   정작 처음 보이는 배(A1)보다 나중에 오지도 않을 그림을 먼저 받고 있었다.
  'A1',                                  // 기함 T0 EMBER — 출격 순간부터 화면에 있다
  'A2', 'A3',                            // 호위 순양함 · 곧 이어지는 승급 선체
  'C1', 'C5',                            // 픽업(크리스탈) · 보급 수송선
...
const REST_PRELOAD = [
  'A4', 'A5', 'A6',
  'H2_BASE_FRAME',                       // §G-48: 기함에서 빠졌다. 폴백 경로용으로만 남긴다
