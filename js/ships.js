...
import { COLORS, makeSprite, blit } from './render.js';
 ...
 */
export const CRUISER_VISUAL_SIZE = 30;
export const DRONE_VISUAL_SIZE = 18;
// 하위 호환 이름. 이제 폭이 아니라 긴 변 표시 크기를 뜻한다.
...
export const SHIP_DEFS = SHIP_DEFS_RAW.map((d, t) => {
  const visualSize = SHIP_VISUAL_SIZE[t] ?? Math.max(d.w, d.h);
  const visualHeight = visualSize;
  const visualWidth = Math.round(visualSize * SHIP_ART_ASPECT[t]);
  const mounts = NORMALIZED_MOUNTS[t].map(([x, y]) => ({ x: x * visualWidth, y: y * visualHeight }));
  const nozzles = NORMALIZED_ENGINES[t].map(([x, y]) => ({ x: x * visualWidth, y: y * visualHeight, len: 5 + t * 1.3 }));
  const deckLights = t >= 3 ? [[-visualWidth * 0.28, 0], [visualWidth * 0.28, 0]] : undefined;
  const formationRadius = Math.round(Math.max(visualWidth * 0.52, visualSize * 0.38));
  return {
    ...d,
    ...
    w: visualWidth, h: visualHeight, clearR: formationRadius,
 ...
 */
export function cruiserBlitScale() { return CRUISER_VISUAL_SIZE / SHIP_DEFS[1].visualSize; }
export function droneBlitScale() { return DRONE_VISUAL_SIZE / SHIP_DEFS[0].visualSize; }
...
export function drawHullFrame(ctx, tier, weapon, previousWeapon = null, blend = 1) {
  ...
  if (!current) return;
  const p = Math.max(0, Math.min(1, blend));
  if (previousWeapon && previousWeapon !== weapon) {
    const previous = frameFor(previousWeapon);
    if (previous) { ctx.save(); ctx.globalAlpha = 1 - p; blit(ctx, previous, 0, 0); ctx.restore(); }
  }
  ctx.save(); ctx.globalAlpha = p; blit(ctx, current, 0, 0); ctx.restore();
}

export function weaponMountSpriteId(weapon, evolutionId = null) {
  if (weapon === 'laser') return evolutionId === 'laser_cutter' ? 'MOUNT_LASER_CUTTER' : evolutionId === 'laser_prism' ? 'MOUNT_LASER_PRISM' : 'MOUNT_LASER_BASE';
  if (weapon === 'homing') return evolutionId === 'homing_wasp' ? 'MOUNT_HOMING_WASP' : evolutionId === 'homing_siege' ? 'MOUNT_HOMING_SIEGE' : 'MOUNT_HOMING_BASE';
...
export function drawWeaponRig(ctx, tier, weapon, weaponLv, t, evolutionId = null, superId = null) {
  ...
  const mountArt = getSprite(weaponMountSpriteId(weapon, evolutionId));
  for (let i = 0; i < mountCount; i++) {
    const m = d.mounts[i];
    if (mountArt) {
      const artScale = (0.34 + tier * 0.055 + weaponLv * 0.035) * (advanced ? 1.08 : 1) * (superAdvanced ? 1.08 : 1);
      // 이미지 하단 소켓이 하드포인트에 닿도록 중심을 위로 올린다.
      blit(ctx, mountArt, m.x, m.y - mountArt.logicalH * artScale * 0.34, artScale);
    } else if (weapon === 'laser') drawLaserRig(ctx, m, size, t, advanced || superAdvanced);
...
export function drawUpgradeSequence(ctx, tier, fx) {
  ...
  ctx.save(); ctx.globalCompositeOperation = 'lighter';
  const artId = fx.kind === 'flagship'
    ? (elapsed < 0.24 ? 'VFX_CORE' : elapsed < 0.66 ? 'VFX_ARMOR' : 'VFX_TIER')
    : (fx.kind === 'evolution' || fx.kind === 'super') ? 'VFX_WEAPON' : 'VFX_CORE';
  const art = getSprite(artId);
  if (art) {
    const fade = Math.sin(Math.PI * elapsed);
    ctx.save();
    ctx.globalAlpha = Math.max(0, fade) * (fx.kind === 'weapon' ? 0.55 : 0.82);
    ctx.rotate((elapsed - 0.5) * (artId === 'VFX_CORE' ? 0.18 : 0.06));
    blit(ctx, art, 0, 0, 0.62 + elapsed * 0.48);
    ctx.restore();
  }
  if (elapsed < 0.28) {
