...
test('위상 잔상은 3번째 graze마다 발동하고 반경 내 거리순 최대 8발만 제거', () => {
  ...
  s.onGrazeKeystone(w, {}); s.onGrazeKeystone(w, {});   // 1,2 → 발동 안 함
  const deadBefore = w.enemyBullets.filter((b) => b.dead).length;
  assert.equal(deadBefore, 0);
  s.onGrazeKeystone(w, {});                              // 3 → 파동
...
test('위상 잔상이 제거한 탄은 이미 dead → 이후 graze 미지급 대상', () => {
  const s = sq(); s.keystone = 'phase_afterimage'; s.keystoneState = freshKeystoneState();
  const w = makeWorld(s);
  for (let i = 0; i < 5; i++) w.enemyBullets.push({ x: s.x + i, y: s.y - 10, r: 6, dead: false });
  s.onGrazeKeystone(w, {}); s.onGrazeKeystone(w, {}); s.onGrazeKeystone(w, {});
  assert.ok(w.enemyBullets.every((b) => b.dead), '반경 내 5발 모두 제거(dead) → graze 대상 아님');
});
