...
test('G2-18: 캠페인 play 완전 무기 선택제(이사 요청, G2-G)', () => {
  ...
  // 선택창 헬퍼(공용 pickCard): auto=true 즉시 자동(autoId), 아니면 정지+카드. campaignPick은 측정(cl.auto)·cl부재면 자동으로 넘기는 래퍼.
  assert.ok(mainSrc.includes('function pickCard') && /function pickCard[\s\S]{0,160}if \(auto\)[\s\S]{0,90}onPick\(autoId/.test(mainSrc), '선택창(공용): auto면 자동 선택');
  assert.ok(/function campaignPick[\s\S]{0,140}pickCard\(\{ \.\.\.opts, auto: !cl \|\| cl\.auto/.test(mainSrc), '캠페인 픽=측정/무캠페인 자동 래퍼');
  ...
  assert.ok(/drafting = true; if \(cl\) cl\.picking = true; state = 'play';\s*\n\s*if \(run\.squad\) run\.squad\.invulnT = Math\.max/.test(mainSrc), '4차: 선택창 여는 프레임 무적(공용 pickCard)');
