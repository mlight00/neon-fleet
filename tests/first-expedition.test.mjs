...
// ─── 9. 첫 노드 복제 수는 최대 2다 ───────────────────────────
test('copyCount: 튜토리얼은 어떤 난이도에서도 최대 2', () => {
  for (const d of [1, 1.5, 2, 5, 10, 30]) {
    assert.ok(copyCount(d, true) <= 2, `difficulty ${d} 튜토리얼 복제 ${copyCount(d, true)}`);
  }
  assert.equal(copyCount(1, true), 2);        // 첫 노드(난이도 1): base 2 → 유지
});

test('copyCount: 비-튜토리얼은 난이도에 따라 2→8 증가, 상한 8 (§4.5)', () => {
  assert.equal(copyCount(1, false), 2);       // 첫 섹터 = 2
  assert.equal(copyCount(3, false), 3);       // 2 + floor((3-1)/2)
  assert.equal(copyCount(999, false), 8);     // 상한
  assert.ok(copyCount(7, false) >= copyCount(1, false));
});
