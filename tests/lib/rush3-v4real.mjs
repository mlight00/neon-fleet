...
//  1~24 × evLead·aimLead·planBoss 한 판씩(랜덤 길 = 기본 시드, 상한 V4_MAX_STEPS — r4.7 부터 28,800 STEP) 돌린 결과를 한 파일(tests/fixtures/rush3-v4-real.json)로 잠근다.
...
//  한 판 상한(STEP). r4.7 재기준: 14,400 → 28,800(8분) — 보스 체력 바닥(30초 × 상한 화력)으로 판이 길어져, '끝나지 않는 판 0'(동작 확인 — 난이도 판단 아님)을
//   명세의 상한(28,800)으로 이 기준값 파일이 함께 잠근다(V4-REAL 검사가 모든 판 over 를 확인)
export const V4_MAX_STEPS = 28800;
...
export function v4RealRun(id, bot, runOpts = V4_RUN_OPTS) {
  return v4RealPick(playPolicy(id, bot, V4_MAX_STEPS, 'brutal', undefined, runOpts));
}
/** 기준값 파일 내용 전체 */
export function v4RealFixture() {
  ...
  return {
    meta: { source: 'r4.7 — createRun(buildStage(id, { difficulty: "brutal" }), { heroGuard: true }), 강화 0, playPolicy 상한 ' + V4_MAX_STEPS + ' STEP, 랜덤 길 기본 시드',
