...
test('V3-BRUTAL 지옥 전용 추가 배치: extraSpawns(옛 brutalSpawns)는 기본 줄(brutal)에서만 스폰 목록에 들어가고, 배수 1 줄(normal) 판은 추가 전과 같다', () => {
  ...
  for (const id of withExtra) {
    ...
    //  r4.10 대물결 겹(horde — 게임 줄 대물결 판 1·4·7·…)·현상금 적(bounty)은 추가 배치가 아니다(따로 센다)
    const extraN = buildStage(id, { difficulty: 'brutal' }).spawns.filter((s) => s.kind !== 'bounty' && !s.horde).length - buildStage(id, { difficulty: 'normal' }).spawns.length;
  ...
  for (const id of withExtra) {
    const base = buildStage(id, { difficulty: 'normal' }).spawns, brutal = buildStage(id, { difficulty: 'brutal' }).spawns.filter((s) => s.kind !== 'bounty' && !s.horde);
  ...
  //  r4.10 대물결 겹(horde — 게임 줄 대물결 판의 끝맺음, 검사 HORDE)도 따로 센다
  const noBounty = (list) => list.filter((s) => s.kind !== 'bounty' && !s.horde);
  for (const id of ALL_STAGE_IDS) {
