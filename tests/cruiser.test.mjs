...
test('집중 게이지는 적 처치로 차며 순양함 수와 무관하다 (콤보 방식·후반에도 획득)', () => {
  const a = squadWithCruisers(0), b = squadWithCruisers(8);
  const wa = makeWorld(a), wb = makeWorld(b);
  a.onEnemyKill(wa, { isEnemy: true }); a.onEnemyKill(wa, { isEnemy: true });
  b.onEnemyKill(wb, { isEnemy: true }); b.onEnemyKill(wb, { isEnemy: true });
  assert.equal(a.flow, b.flow);                    // 순양함 0척/8척 동일
  assert.equal(a.flow, BAL.flow.gainPerKill * 2);
});
