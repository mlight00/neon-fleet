...
import { planAttack, unlockedAtk } from '../rush3/bossatk.js';
...
test('BOSS-RAGE 1·2번: 페이즈가 없는 학습 구간도 광분은 켠다(이벤트·bo.rage) — 페이즈는 0 그대로라 새 공격은 열리지 않는다(삽날 밀기·잔해 튕기기 두 가지 그대로)', () => {
  for (const id of [1, 2]) {
    const run = bossRun(id);
    steps(run, 60 * 3);
    const bo = run.bosses[0];
    ...
    assert.deepEqual(unlockedAtk(bo.atk, bo.phase), ['blade', 'ricochet'], `S${id}: 공격 그대로`);
  }
});

test('BOSS-RAGE 배수: 광분 중 다음 공격까지 간격 = 스킨 간격 × 페이즈 rate × 0.8 · 탄 공격의 탄 속도 × 1.12(같은 자리의 같은 공격 설계로 비교) · 광역 경보 시간은 그대로', () => {
  //  간격: 4번 B1 — 광분 전(페이즈 2 를 강제로) · 광분 뒤 공격이 끝난 STEP 의 대기 시간
  for (const rage of [false, true]) {
    const { run, bo } = atkRun(4, 30, 0, 'blade', 240, 0, { mod: (r, b) => { b.phase = 2; if (rage) b.rage = true; } });
    let end = null;
  ...
  for (const [id, kind] of [[4, 'blade'], [4, 'ricochet'], [6, 'needles'], [11, 'chain'], [16, 'slag'], [24, 'blades']]) {
  ...
  //  광역 경보는 그대로
  for (const [id, kind] of [[6, 'hook'], [11, 'rail'], [16, 'pour'], [24, 'quake']]) {
...
test('BOSS-RAGE 셸: 광분에 들어가는 프레임에 화면 가운데 \'광분!\' 배너(한 어절) + 경고음(bossRage) + 흔들림 · 약 1초 뒤 배너가 사라진다 · 같은 STEP 의 페이즈 글자(보스 광분!)는 두 번 알리지 않는다 · 체력 막대가 붉다', async () => {
  const h = await bootApp({ withOps: true, unlockThrough: 3 });
  h.app.startRun(4);
  h.frames(2);
...
test('BOSS-RAGE 한 번에 한 공격·경보 그대로: 광분(1·4·10·24번) 20초 — 시작~끝 사이에 새 공격이 없고, 광역 경보는 0.7초 이상, 보이는 탄·경보로 설 곳에 간 봇은 피해 0', () => {
  for (const id of [1, 4, 10, 24]) {
